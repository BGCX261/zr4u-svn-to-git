...
class Router {
	...
	/**
	 * Router setup routine. Automatically called during Lemon setup process.
	 *
	 * @return  void
	 */
	public static function setup()
	{
		...
		if (Router::$current_uri === '')
		{
			...
			if ( ! isset(Router::$routes['_default']))
				throw new LemonRuntimeException('core.no_default_route', 500);
	...
	public static function find_uri()
	{
		...
		elseif (isset($_GET['lemon_uri']))
		{
			// Use the URI defined in the query string
			Router::$current_uri = $_GET['lemon_uri'];

			// Remove the URI from $_GET
			unset($_GET['lemon_uri']);

			// Remove the URI from $_SERVER['QUERY_STRING']
			$_SERVER['QUERY_STRING'] = preg_replace('~\blemon_uri\b[^&]*+&?~', '', $_SERVER['QUERY_STRING']);
		}
		...
		if (($strpos_fc = strpos(Router::$current_uri, LEMON)) !== FALSE)
		{
			// Remove the front controller from the current uri
			Router::$current_uri = (string) substr(Router::$current_uri, $strpos_fc + strlen(LEMON));
		}
		...
		if (Router::$current_uri !== '')
		{
			if ($suffix = Lemon::config('core.url_suffix') AND strpos(Router::$current_uri, $suffix) !== FALSE)
			{
